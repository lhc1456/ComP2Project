...
void display_population(population *pop)
{
  ...
  for(i=0; i<pop->pop_size; i++)
  {
    ...
    printf(" fitness : %f", pop->ivd[i].fitness);
    printf("\n");
  }
}

/*돌연변이, 랜덤으로 값을 뒤바꾸어 준다. make : 151107, adjust : 151215*/
void mutate(chromosome *crms, int bit_count)
{
  int i;
...
/*적응도 함수로 개체에 적응도를 더해줍니다. 일정 횟수를 지나면 돌연변이를 가해줍니다. make : 151108, adjust : 151215*/
void fitness_func(population *pop, int x, int y, int count)
{
  int i;
  double fit;
...
/*플레이어와 컴퓨터가 순서대로 돌을 둔다. make : 151109, adjust : 151201, adjust : 151215*/
void play_board(population *pop, int (*board)[15])
{
  int i, j,
      x, y,
  ...
  while(1)
  {
    ...
    board[y][x] = 'O';
		...
		//board에 집어 넣은 O와 X를 좌표에 맞게 출력하여 줍니다. 
    printf("   ");
    for(i=0; i<15; i++)
    {
      printf("%2d ", i);
...
/*직각삼각형의 대각선을 구하는 함수입니다. x, y가 각각 직각삼각형의 밑변과 높이입니다.make : 151202*/
